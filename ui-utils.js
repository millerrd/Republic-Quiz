...
function showOptionResults(selectedAnswer, correctAnswer) {
    if (currentPhase === 'bonus' && bonusType === 'flag') {
        // Special handling for flag options using data attributes
        var flagOptions = document.querySelectorAll('#options .flag-option');
        
        for (var i = 0; i < flagOptions.length; i++) {
            flagOptions[i].classList.add('disabled');
            
            var flagCode = flagOptions[i].getAttribute('data-flag');
            
            if (flagCode === selectedAnswer) {
                flagOptions[i].classList.add('selected');
            }
            if (flagCode === correctAnswer) {
                flagOptions[i].classList.add('correct');
            }
            if (flagCode === selectedAnswer && selectedAnswer !== correctAnswer) {
                flagOptions[i].classList.add('incorrect');
            }
        }
    } else {
        // Regular handling for text options
